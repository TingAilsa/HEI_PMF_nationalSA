...
PATH_ROOT     <- "/scratch"
PATH_TEMPLATE <- file.path(PATH_ROOT, "raw/template/template_001.tif")
PATH_POINTS   <- file.path(PATH_ROOT, "raw/points/us_point_coord.fst")
PATH_PRED     <- file.path(PATH_ROOT, "predictors")    # per-variable, per-day fst shards
PATH_DAILY    <- file.path(PATH_ROOT, "daily_grids")   # final assembled daily predictor files
...
resample_to_template <- function(r, template, method) {
...
save_var_day <- function(values, varname, date) {
...
extract_and_write_batch <- function(r_batch, dates, pts_v, varname) {
...
run_gridmet <- function() {
  ...
  process_gridmet_var <- function(file_prefix, varname, year) {
