...
PM_avg = ddply(species_daily, 
               .(SiteCode), 
               summarise,
               PM25 = mean(PM25))

PM_avg = merge(PM_avg, pm_cluster)

# the mean PM level of each cluster
PM_cluster = ddply(PM_avg, 
                   .(Final.Decision), 
                   summarise,
                   PM25 = mean(PM25))
...
cty_rural_urban = read.csv("/Users/ztttttt/Dropbox/HEI_PMF_files_Ting/National_SA_data_prepare/IMPROVE_CSN_PopDensity_Urban_Rural_classify_331sites.csv")
cty_rural_urban$X = cty_rural_urban$X.1 = NULL

site_gps = select(cty_rural_urban, SiteCode, Longitude, Latitude)

PM_site = merge(PM_avg, site_gps)
...
sites.top.PM.cluster = PM_avg$SiteCode[
  which(
    PM_avg$Final.Decision == 
      cluster.top.PM)]
sites.least.PM.cluster = PM_avg$SiteCode[
  which(
    PM_avg$Final.Decision == 
      cluster.least.PM)]
...
species_daily_conc$State = species_daily_conc$Qualifier = NULL
...
species_mdl = fread("/Users/ztttttt/Documents/HEI PMF/CSN_IMPROVE/CSN_MDL_C-Sub_monthly_2023.05.csv")
species_mdl$V1 = species_mdl$Cl. = NULL # csn_mdl$OP = 

### IMPROVE
species_mdl = fread("/Users/ztttttt/Documents/HEI PMF/R - original IMPROVE/IMPROVE_MDL_monthly_2023.csv")
...
comp_error_fraction = fread("/Users/ztttttt/Documents/HEI PMF/IMPROVE & CSN original/CSN_k_Error-Fraction_2023.04.csv")
...
unc_pmf = species_conc_mdl * (species_mdl / 3 + comp_ef * species_conc) +
...
conc_pmf_randomsite_check = subset(conc_pmf, SiteCode == "BADL1")
...
####################################################################
###### Origin_MDL - data for PMF Non-GUI - all values ###### 
####################################################################
...
#### CSN
# cluster_snr = read.csv("pm_cluster_SNR_for_PMF.csv") # No carbon subgroups
...
################################################################################
#### data for PMF - NO EXTREMES! for GUI & noGUI - Clusters ####
################################################################################
...
conc_pmf = fread("CSN_concentration_AQS.PM_PMF_C-sub_2023.05.csv")
unc_pmf = fread("CSN_uncertainty_AQS.PM_PMF_C-sub_2023.05.csv")
conc_pmf$Final.Decision = unc_pmf$Final.Decision = NULL
...
for( i in 1:25){
  ...
  ##### determine the extremes 1.1 - mean*25 #####
  conc_cluster_select = conc_cluster
  # summary(conc_cluster_select)
  mean_conc_cluster <- conc_cluster_select[, lapply(.SD, mean), .SDcols = 4:ncol(conc_cluster)]
  ...
  # identify rows in conc_cluster_select with values higher than the extreme values
  # rows_to_remove_25mean <- apply(conc_cluster_select[, 4:ncol(conc_cluster_select)], 1, 
  #                        function(row) 
  #                          any(row > extreme_conc_cluster))
  
  # expand the extreme rows to the same as conc_cluster
  extreme_conc_cluster_exp = 
    extreme_conc_cluster %>% 
    slice(rep(1:n(), each = nrow(conc_cluster_select)))
  extreme_conc_cluster_exp$Final.Decision = NULL
  
  thresholds_25TimesMean = rbind(thresholds_25TimesMean, extreme_conc_cluster)
  write.csv(thresholds_25TimesMean, "CSN_thresholds_25TimesMean.csv")
  

  ######## combine the 25mean threshold info with daily time sereis
  conc_cluster_select$Season = season_date(conc_cluster_select$Date)
  conc_cluster_select$Year = year(conc_cluster_select$Date)
  
  conc_cluster_select_identifyInfo = select(conc_cluster_select,
                                            State, SiteCode, Final.Decision, 
                                            Year, Season, Date)
  conc_cluster_25mean_daily = cbind(conc_cluster_select_identifyInfo,
                                    extreme_conc_cluster_exp)
  
  conc_cluster_extreme_daily = conc_cluster_25mean_daily
  
  ##### determine the extremes 1.2 - seasonal 99th #####
  conc_cluster_select = conc_cluster
  conc_cluster_select$Season = season_date(conc_cluster_select$Date)
  conc_cluster_select$Year = year(conc_cluster_select$Date)
  ...
  conc_cluster_season99 =
    ddply(conc_cluster_long, 
  ...
  thresholds_Seasonal99th = rbind(thresholds_Seasonal99th, conc_cluster_season99)
  ...
  conc_cluster_select_identifyInfo = select(conc_cluster_select,
                                            State, SiteCode, Final.Decision, 
                                            Year, Season, Date)
  conc_cluster_season99_daily = merge(conc_cluster_select_identifyInfo,
                                      conc_cluster_season99,
                                      all.x = T)
  
  conc_cluster_extreme_daily = conc_cluster_season99_daily
  
  
  ##### determine the extremes 2 - remove selected rows with extremes #####
  # this part, mostly focus on days with firework on holidays

  # reorder the columns and rows
  conc_cluster_extreme_daily = select(conc_cluster_extreme_daily, 
                                      names(conc_cluster_select))
  conc_cluster_extreme_daily = 
    conc_cluster_extreme_daily[with(
      conc_cluster_extreme_daily, 
      order(State, SiteCode, Date)), ]
  
  # identify rows in conc_cluster with values higher than the extreme values
  conc_cluster_extreme_comp = data.frame(conc_cluster_select > conc_cluster_extreme_daily)
  
  conc_cluster_extreme_comp = 
    conc_cluster_extreme_comp[with(
      conc_cluster_extreme_comp, 
      order(State, SiteCode, Date)), ]
  
  conc_cluster_extreme_comp$SiteCode = conc_cluster_select$SiteCode
  conc_cluster_extreme_comp$Date = conc_cluster_select$Date
  conc_cluster_extreme_comp$State = conc_cluster_select$State
  conc_cluster_extreme_comp$Final.Decision = conc_cluster_select$'Final Decision'
  conc_cluster_extreme_comp$Year = conc_cluster_extreme_comp$Season = NULL
  
  # mostly focus on days with firework on holidays with high K
  conc_cluster_extreme_comp_K = subset(conc_cluster_extreme_comp, K == T)
  conc_cluster_extreme_comp_K$row.No = row.names(conc_cluster_extreme_comp_K)
  
  # extract rows with extremes
  conc_cluster_extreme_comp_K$Month = month(conc_cluster_extreme_comp_K$Date)
  conc_cluster_extreme_comp_K$Day = 
    as.integer(format(conc_cluster_extreme_comp_K$Date, 
                      format = "%d"))
  conc_cluster_extreme_comp_K_holiday = 
    subset(conc_cluster_extreme_comp_K,
           (Month == 7 & Day %in% 1:6)) # | (Month == 12 & Day %in% 23:31) | (Month == 1 & Day %in% 1:5)
  removed.row.count = nrow(conc_cluster_extreme_comp_K_holiday)
  
  rows_to_remove = 1:nrow(conc_cluster_select) %in% unique(conc_cluster_extreme_comp_K_holiday$row.No)
  sum(rows_to_remove)
  
  removed_rows = conc_cluster[rows_to_remove, ]
  removed_rows$row.No = conc_cluster_extreme_comp_K_holiday$row.No
  ...
  # extract the date and site info of the rows to be removed
  # remove_date_site = select(removed_rows, SiteCode, Date)
  ...
  # extract the conc_vs._mdl data of selected cluster(s)
  conc_mdl_cluster = subset(species_conc_mdl_cluster,
                            SiteCode %in% sites.in.cluster)
  conc_mdl_cluster <- conc_mdl_cluster[!rows_to_remove, ]
  
  # Remove rows in conc_mdl_cluster based on Site and Date groups in remove_date_site
  conc_mdl_cluster <- 
    conc_mdl_cluster %>%
    anti_join(remove_date_site, 
              by = c("SiteCode", "Date"))
  
  conc_mdl_cluster$State = NULL
  dim(conc_mdl_cluster)
  ...
  ## checking the ratio of concentrations above MDL for PMF resetting for selected cluster
  cols_speceis = setdiff(names(conc_mdl_cluster),
                         c("SiteCode", "Date"))
  conc_cluster_aboveMDL = data.frame(round(sapply(
    conc_mdl_cluster[, ..cols_speceis], sum)/
      nrow(conc_mdl_cluster)*100, 0))
  ...
  #   annotate("text", x = 15, y = 20, label = "Weak  ↓", vjust = -0.5, col = "orange") +
  #   annotate("text", x = 15, y =10, label = "Bad  ↓", vjust = -0.5, col = "red") +
  #   ylab("Percent of below-species_MDL concentration") +
  #   theme_bw()
  
  
  ###### Re-estimate signal-to-noise SNR after removing extremes ######
  conc.col = ncol(conc_cluster)
  species_conc_mdl = conc_mdl_cluster[, ..cols_speceis]
  
  # according to EPA PMF 5.0 User Guide.pdf, function 5-3 & 5-4 from page 23
  conc_unc_diff_cluster = (conc_cluster[, ..cols_speceis] - 
                             unc_cluster[, ..cols_speceis])/
    unc_cluster[, ..cols_speceis] * species_conc_mdl
  ...
  cluster.species.bad = append(c("S"), 
                               cluster.species.bad) 
  # in case "S  is in weak, if so, remove
  cluster.species.weak = cluster.species.weak[! (
    cluster.species.weak %in% c("S"))]
  
  
  # add Al, Mg, Na into weak, if there are not in bad, due to the lower reliability in XRF test
  if("NaIon" %in% cols_speceis){
    Al.Mg.weak = c("Al", "Mg")
  } else {
  ...
  ###### Cluster- final 1: files of conc & unc for CMD ######
  # remove species marked as bad
  conc_cluster_pmf <- conc_cluster[, !cluster.species.bad, with=FALSE]
  unc_cluster_pmf <- unc_cluster[, !cluster.species.bad, with=FALSE]
  
  ## output files for PMF GUI
  write.csv(conc_cluster_pmf,
            paste0(GUI.cluster.folder, "/", prefix, 
  ...
  # unc_cluster_pmf[names(unc_cluster_pmf) %in% cluster.species.weak] = 
  #   3 * unc_cluster_pmf[names(unc_cluster_pmf) %in% cluster.species.weak]
  unc_cluster_pmf[, (cluster.species.weak) := 
                    lapply(.SD, 
                           function(x) 
                             x * 3), 
                  .SDcols = cluster.species.weak]
  
  # for PM2.5, set the uncertainty 3*3 times as much
  unc_cluster_pmf$PM25 = 3 * conc_cluster_pmf$PM25
  ...
  ## reorder the cbind file
  # cmd_cluster_interleave = cmd_cluster_conc_unc[interleave.cluster] # for data.frame only
  cmd_cluster_interleave = 
    as.data.table(
      as.matrix(
        cmd_cluster_conc_unc)[, 
                              interleave.cluster])
  
  # only keep one Date & cluster column
  cmd_cluster_interleave$unc_Date = 
    cmd_cluster_interleave$unc_State = 
    cmd_cluster_interleave$unc_SiteCode = NULL
  colnames(cmd_cluster_interleave)[1:3] = 
    c("SiteCode", "Date", "State")
  head(cmd_cluster_interleave)
