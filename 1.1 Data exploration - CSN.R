...
ggplot(subset(csn_Br, SiteCode == 10730023), 
       aes(x = day, y = MDL, shape = as.factor(year)))+ # color = as.factor(year)
  geom_point(alpha = 0.8, size = 2) +
  ggtitle("Br - MDL") + theme_mdl

ggplot(subset(csn_Br, SiteCode == 10730023), 
       aes(x = as.factor(year), y = MDL, color = day))+
  geom_violin() + 
...
csn_component_use$method.No = 
  csn_component_use$dup + csn_component_use$dup.1 + csn_component_use$dup.2 + 1
csn_component_use$method.No[
  csn_component_use$ParamCode %in% 
    csn_component_use$ParamCode[csn_component_use$method.No==4]] = 4
csn_component_use$method.No[
  csn_component_use$ParamCode %in% 
    csn_component_use$ParamCode[csn_component_use$method.No==3]] = 3
csn_component_use$method.No[
  csn_component_use$ParamCode %in% 
    csn_component_use$ParamCode[csn_component_use$method.No==2]] = 2
...
sum(csn_qualifier1$Qualifier1 != "---") == nrow(csn_qualifier1)
sum(csn_qualifier1$Qualifier2 != "---") == nrow(csn_qualifier1)
...
csn_qualifier_only1 = 
  subset(csn_qualifier, Qualifier1 != "---" & 
           Qualifier2 == "---")
...
csn_qualifier_only1 = subset(csn_qualifier_only1, s
                             elect = c(serial.No, Qualifier1))

csn_qualifier_both12 = 
  subset(csn_qualifier, 
         csn_qualifier$Qualifier2 != "---" & 
           csn_qualifier$Qualifier3 == "---")
csn_qualifier_both12$Qualifier3 = NULL
csn_qualifier_both12$serial.No = 1:nrow(csn_qualifier_both12)
csn_qualifier_both12 = 
  subset(csn_qualifier_both12, 
         select = c(serial.No, Qualifier1, Qualifier2))

csn_qualifier_all123 = csn_qualifier1
csn_qualifier_all123$serial.No = 1:nrow(csn_qualifier_all123)
csn_qualifier_all123 = 
  subset(csn_qualifier_all123, 
         select = c(serial.No, Qualifier1, Qualifier2, Qualifier3))

csn_qualifier_only1$qualifier.code = "Qualifier1"; colnames(csn_qualifier_only1)[2] = "qualifier.type"
csn_qualifier_only1 = 
  subset(csn_qualifier_only1, 
         select = c(serial.No, qualifier.code, qualifier.type))
csn_qualifier_both12 = 
  gather(csn_qualifier_both12, "qualifier.code", "qualifier.type", -serial.No)
csn_qualifier_all123 = 
  gather(csn_qualifier_all123, "qualifier.code", "qualifier.type", -serial.No)
...
# frequency of different qualifiers
Qualifier_freq_plot_use_1 = 
  select(Qualifier_freq_plot, 
         Qualifier.Code, Qaulifier.Type.Code,
         Freq.total, Freq.1flag, 
         Freq.2flag, Freq.3flag)
View(Qualifier_freq_plot_use_1)

Qualifier_freq_INFORM_use = 
  subset(Qualifier_freq_plot_use_1, Qaulifier.Type.Code == "INFORM")
Qualifier_freq_QA_use = 
  subset(Qualifier_freq_plot_use_1, Qaulifier.Type.Code == "QA")
Qualifier_freq_INFORM_use$Qaulifier.Type.Code = 
  Qualifier_freq_QA_use$Qaulifier.Type.Code = NULL

Qualifier_freq_INFORM_use = 
  gather(Qualifier_freq_INFORM_use, "Freq_flag_counts", "Frequency", -Qualifier.Code)
Qualifier_freq_QA_use = 
  gather(Qualifier_freq_QA_use, "Freq_flag_counts", "Frequency", -Qualifier.Code)
...
#### Below are flags potentially INFLUENCE the component values!!!!! according to EPA qualifier explanation
...
csn_flag$QA = 
  ifelse(csn_flag$qa1, 
         csn_flag$Qualifier1, csn_flag$QA)
csn_flag$QA = 
  ifelse(!csn_flag$qa1 & csn_flag$qa2, 
         csn_flag$Qualifier2, csn_flag$QA)
csn_flag$QA = 
  ifelse(!csn_flag$qa1 & !csn_flag$qa2 & csn_flag$qa3, 
         csn_flag$Qualifier3, csn_flag$QA)
summary(is.na(csn_flag$QA))

# generate data of species concentrations
...
INFORM_flag = 
  unique(Qualifier_freq_EPA$Qualifier.Code[
    Qualifier_freq_EPA$Qaulifier.Type.Code == "INFORM"])  
INFORM_flag = INFORM_flag[!is.na(INFORM_flag)]

Qualifier_freq_EPA$percent = 
  label_percent()(round(Qualifier_freq_EPA$Freq.total/nrow(csn_data), 5))
...
component_summary = comp_concentration = comp_ratio_mean = comp_ratio_median = 
  comp_concen_summary = component_name
...
# summary the distribution of qualifiers
# for(k in 1:2){
for(k in 1:length(all_flag)){
  flag_qa = as.character(all_flag[k])
  percent_summary = 
    qualifier.info$percent[qualifier.info$Qualifier.Code == flag_qa]
  qualifier_expl = 
    qualifier.info$Qualifier.Description[
      qualifier.info$Qualifier.Code == flag_qa]
  single_qa = 
    subset(csn_flag_check, 
           Qualifier1 == flag_qa | 
             Qualifier2 == flag_qa | 
             Qualifier3 == flag_qa)
  ...
  if(nrow(single_qa) > 0) {
    ...
    comp_ratio_cal = 
      cbind(comp_concentration_qa[1], 
            round(comp_concentration_qa[-1]/comp_concen_no_flag[-1],3))
...
# ratios of mean concentrations, flagged vs. not
comp_ratio_mean_ga = 
  comp_ratio_mean %>% 
  gather(QA, mean.ratio, -CompName)
comp_ratio_mean_ga$qualifier = 
  sub("\\-.*", "", comp_ratio_mean_ga$QA)
...
# ratios of median concentrations, flagged vs. not
comp_ratio_median_ga = 
  comp_ratio_median %>%
  gather(QA, median.ratio, -CompName)
comp_ratio_median_ga$qualifier = 
  sub("\\-.*", "", comp_ratio_median_ga$QA)
...
# qualifiers of different frequency
qa_most = 
  unique(Qualifier_freq_EPA$Qualifier.Code[
    Qualifier_freq_EPA$Freq.total > 55000])
qa_most = qa_most[2:length(qa_most)]
qa_more_10k = 
  unique(Qualifier_freq_EPA$Qualifier.Code[
    Qualifier_freq_EPA$Freq.total < 55000 &
      Qualifier_freq_EPA$Freq.total > 10000])
qa_1k_10k = 
  unique(Qualifier_freq_EPA$Qualifier.Code[
    Qualifier_freq_EPA$Freq.total < 10000 &
      Qualifier_freq_EPA$Freq.total > 1000])
qa_less_1k =
  unique(Qualifier_freq_EPA$Qualifier.Code[
    Qualifier_freq_EPA$Freq.total < 1000])
length(qa_most) + length(qa_more_10k) + length(qa_1k_10k) + length(qa_less_1k)

# annual distribution
qa_year_sum = 
  year_summary %>% 
  gather(QA, count, -year)
qa_year_sum$count[is.na(qa_year_sum$count)] = 0
qa_year_sum$year = as.factor(qa_year_sum$year)

year_most = 
  ggplot(subset(qa_year_sum, qa_year_sum$QA %in% qa_most), 
         aes(year, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "red") +
  theme.2
year_more_10k = 
  ggplot(subset(qa_year_sum, qa_year_sum$QA %in% qa_more_10k), 
         aes(year, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "purple") +
  theme.2
year_1k_10k = 
  ggplot(subset(qa_year_sum, qa_year_sum$QA %in% qa_1k_10k), 
         aes(year, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "deepskyblue3") +
  theme.2
year_less_1k = 
  ggplot(subset(qa_year_sum, qa_year_sum$QA %in% qa_less_1k), 
         aes(year, QA, fill= count)) + 
  geom_tile() + 
...
qa_month_sum$month = as.factor(qa_month_sum$month)

month_most = 
  ggplot(subset(qa_month_sum, qa_month_sum$QA %in% qa_most), 
         aes(month, QA, fill= count)) + 
  geom_tile() + scale_fill_gradient(low = "white", high = "red") +
  theme.2
month_more_10k = 
  ggplot(subset(qa_month_sum, qa_month_sum$QA %in% qa_more_10k), 
         aes(month, QA, fill= count)) + 
  geom_tile() + scale_fill_gradient(low = "white", high = "purple") +
  theme.2
month_1k_10k = 
  ggplot(subset(qa_month_sum, qa_month_sum$QA %in% qa_1k_10k), 
         aes(month, QA, fill= count)) + 
  geom_tile() + scale_fill_gradient(low = "white", high = "deepskyblue3") +
  theme.2
month_less_1k = 
  ggplot(subset(qa_month_sum, qa_month_sum$QA %in% qa_less_1k), 
         aes(month, QA, fill= count)) + 
...
state_most = 
  ggplot(subset(qa_state_sum, qa_state_sum$QA %in% qa_most), 
         aes(State, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "red")
state_more_10k = 
  ggplot(subset(qa_state_sum, qa_state_sum$QA %in% qa_more_10k), 
         aes(State, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "purple")
state_1k_10k = 
  ggplot(subset(qa_state_sum, qa_state_sum$QA %in% qa_1k_10k), 
         aes(State, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "deepskyblue3")
state_less_1k = 
  ggplot(subset(qa_state_sum, qa_state_sum$QA %in% qa_less_1k), 
         aes(State, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "green")
...
comp_most = 
  ggplot(subset(qa_comp_sum, qa_comp_sum$QA %in% qa_most),
         aes(CompName, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "red") + 
  theme.2
comp_more_10k = 
  ggplot(subset(qa_comp_sum, qa_comp_sum$QA %in% qa_more_10k), 
         aes(CompName, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "purple") + 
  theme.2
comp_1k_10k = 
  ggplot(subset(qa_comp_sum, qa_comp_sum$QA %in% qa_1k_10k), 
         aes(CompName, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "deepskyblue3") + 
  theme.1
comp_less_1k = 
  ggplot(subset(qa_comp_sum, qa_comp_sum$QA %in% qa_less_1k), 
         aes(CompName, QA, fill= count)) + 
  geom_tile() + 
  scale_fill_gradient(low = "white", high = "green") +
  theme.1
...
site_gps = read.csv("site GPS question or not.csv") # question site, those not in mainland US
csn_no_flag_1 = 
  join(csn_no_flag, 
       select(site_gps,
              State, SiteCode, StartDate, 
              start.year, question.site))
dim(csn_no_flag_1)
...
csn_no_flag_1_plot = 
  subset(csn_no_flag_1, 
         !(csn_no_flag_1$State %in% exclude_state))
csn_no_flag_1_plot$start.year_use = 
  ifelse(csn_no_flag_1_plot$start.year >2014,csn_no_flag_1_plot$start.year, NA)
...
csn_no_flag_species_median =
  csn_no_flag_1_plot %>%
  dplyr::group_by(State, SiteCode, start.year_use, question.site, CompName) %>%
  dplyr::summarise(Val.median = median(Val, na.rm = TRUE),
...
ggplot(subset(csn_no_flag_species_median, 
              # no_flag_SO4_median$Val.median > 0.2 & 
              CompName %in% unique(csn_no_flag_species_median$CompName)[30:35]), 
       aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  facet_wrap(CompName ~., ncol = 3) +
...
# other species, SO4
no_flag_SO4_median = subset(csn_no_flag_species_median,
                            CompName == "SO4")

SO4_median = 
  ggplot(subset(no_flag_SO4_median, 
                # no_flag_SO4_median$Val.median > 0.2 & 
                  is.na(no_flag_SO4_median$start.year_use)), 
                    aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
# other species, NO3
no_flag_NO3_median = subset(csn_no_flag_species_median,
                            CompName == "NO3")

NO3_median = 
  ggplot(subset(no_flag_NO3_median, 
                # no_flag_NO3_median$Val.median > 0.2 & 
                is.na(no_flag_NO3_median$start.year_use)), 
         aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
## Na
# other species, Na
no_flag_Na_median = subset(csn_no_flag_species_median,
                            CompName == "Na")

Na_median = 
  ggplot(subset(no_flag_Na_median, 
                # no_flag_Na_median$Val.median > 0.2 & 
                is.na(no_flag_Na_median$start.year_use)), 
         aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
## Fe
# other species, Fe
no_flag_Fe_median = subset(csn_no_flag_species_median,
                            CompName == "Fe")

Fe_median = 
  ggplot(subset(no_flag_Fe_median, 
                # no_flag_Fe_median$Val.median > 0.2 & 
                is.na(no_flag_Fe_median$start.year_use)), 
         aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
## Cd
# other species, Cd
no_flag_Cd_median = subset(csn_no_flag_species_median,
                            CompName == "Cd")

Cd_median = 
  ggplot(subset(no_flag_Cd_median, 
                # no_flag_Cd_median$Val.median > 0.2 & 
                is.na(no_flag_Cd_median$start.year_use)), 
         aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
## As
no_flag_As_median = subset(csn_no_flag_species_median,
                            CompName == "As")

As_median = 
  ggplot(subset(no_flag_As_median, 
                # no_flag_As_median$Val.median > 0.2 & 
                is.na(no_flag_As_median$start.year_use)), 
         aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
## EC.88
no_flag_EC.88_median = subset(csn_no_flag_species_median,
                            CompName == "EC.88")

EC.88_median = 
  ggplot(subset(no_flag_EC.88_median, 
                # no_flag_EC.88_median$Val.median > 0.2 & 
                is.na(no_flag_EC.88_median$start.year_use)), 
         aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
## OC.88
no_flag_OC.88_median = subset(csn_no_flag_species_median,
                            CompName == "OC.88")

OC.88_median = 
  ggplot(subset(no_flag_OC.88_median, 
                # no_flag_OC.88_median$Val.median > 0.2 & 
                is.na(no_flag_OC.88_median$start.year_use)), 
         aes(Longitude, Latitude, color= Val.median)) + 
  geom_polygon(data=MainStates, aes(x=long, y=lat, group=group),
               color="darkgrey", fill="lightblue", alpha = 0.4) +
  geom_point(size = 2, alpha = 0.6) +
...
########### Mapping the source based on Chemical Mass Reconstruction, CMR #########
...
cmr_no_flag = ddply(csn_no_flag_3, 
                    .(State, CompName), 
                    summarize, 
                    Val = median(Val, na.rm = T)) 
state_gps = ddply(csn_no_flag_3, 
                  .(State), 
                  summarize, 
                  Longitude = median(Longitude, na.rm = T), 
                  Latitude = median(Val, na.rm = T))

cmr_no_flag_spd = cmr_no_flag %>% spread(CompName, Val)
head(cmr_no_flag_spd)
head(cmr_no_flag_spd$Soil)

# other mass reconstruction
cmr_no_flag_spd$Sea.salt = 
  cmr_no_flag_spd$`Cl-` + 1.448*cmr_no_flag_spd$`Na+` # Chow 2015 AQAH
cmr_no_flag_spd$BC = cmr_no_flag_spd$EC.88
cmr_no_flag_spd$POC = 1.8 * cmr_no_flag_spd$OC.88 # CS Liang_2016_EI
cmr_no_flag_spd = join(cmr_no_flag_spd, state_gps)

class(statepov)
head(statepov)
cmr_no_flag_spd_source = 
  data.frame(abbr=cmr_no_flag_spd$State, 
             Soil=cmr_no_flag_spd$Soil, 
             Sea.salt=cmr_no_flag_spd$Sea.salt,
             BC=cmr_no_flag_spd$BC, 
             POC=cmr_no_flag_spd$POC)
statepov_use = as_tibble(join(statepov, cmr_no_flag_spd_source))
class(statepov_use)
head(statepov_use)

plot_usmap(data = statepov_use, labels = T, label_color = "white") +
...
csn_no_flag_2 = 
  csn_no_flag_2[
    with(csn_no_flag_2, 
         order(State, SiteCode, class, Date)), ]
csn_no_flag_2_a = subset(csn_no_flag_2, csn_no_flag_2$class == "element.ion.PM")
...
csn_no_flag$Q.MD = NA
csn_flag_MD$Q.MD = 
  ifelse(csn_flag_MD$Qualifier1 == "MD" | csn_flag_MD$Qualifier2 == "MD" | 
           csn_flag_MD$Qualifier3 == "MD" & csn_flag_MD$multi_qa == 1, T, F)
csn_flag_MD_use = csn_flag_MD
csn_flag_MD_use$qa1 = csn_flag_MD_use$qa2 = csn_flag_MD_use$qa3 = 
  csn_flag_MD_use$QA = csn_flag_MD_use$multi_qa = NULL
...
#############################################################################################################
########### Sampling collection/analysis methods ########### 
##### find the temporal coverage to sampling collection/analysis methods #####

csn_data = fread("CSN data with date 09252022.csv")
...
csn_data_Q = 
  select(csn_data,
         SiteCode, POC, Date, ParamCode, Val, Method,
         Qualifier1, Qualifier2, Qualifier3, Unc, MDL, 
         SampleDuration, State, year, month, day, 
         Collection, Analysis)
head(csn_data_Q)

# number of all qualifiers
...
# number of qualifiers with certain emission information
inform_flag = 
  c("IA",	"IC",	"IE",	"IF",	"IG",	"IH",	"II",	"IJ",	"IK",	"IL",	"IM",	"IN",	"IP",	"IR",	"IS",	"IT",	"J")
...
csn_data_Q$Qualifier.multi.Inform = 
  rowSums(csn_data_Q[, c("Qaulifier.Infm1", "Qaulifier.Infm2", "Qaulifier.Infm3")])
table(csn_data_Q$Qualifier.multi.Inform) 
# 8,307,501, 121,034, 11,710, 359 

# if there is only information related qualifier, & the percent
csn_data_Q$Qualifier.inform.only = 
  ifelse(csn_data_Q$Qualifier_count == csn_data_Q$Qualifier.multi.Inform & csn_data_Q$Q1, T, F)
...
# species measurement method
csn_component_method_use = 
  read.csv("CSN method component reorder 09262022.csv")
csn_component_method_use$X = NULL

csn_data_Q = join(csn_data_Q, csn_component_method_use)
nrow(csn_data_Q)
csn_method = 
  data.frame(State = csn_data_Q$State, SiteCode = csn_data_Q$SiteCode, 
             Date = csn_data_Q$Date, CompName = csn_data_Q$CompName, 
             class = csn_data_Q$class, Serial.No.compo = csn_data_Q$Serial.No.compo,
             Collection = csn_data_Q$Collection, Analysis = csn_data_Q$Analysis, 
             Qualifier1 = csn_data_Q$Qualifier1, Qualifier_count = csn_data_Q$Qualifier_count, 
             Qualifier.inform.only = csn_data_Q$Qualifier.inform.only)
# csn_method$Date = as.POSIXct(as.character(csn_method$Date), format="%m/%d/%Y", tz="EST")
csn_method = 
  csn_method[
    with(csn_method, 
         order(Date, State, SiteCode, class, Serial.No.compo)), ]
head(csn_method)
csn_method$comp.collect.analysis = 
  paste(csn_method$CompName, csn_method$Collection, csn_method$Analysis)
...
comp_collect_analysis$start.date = 
  as.Date(
    as.POSIXct(
      as.character(comp_collect_analysis$start.date), 
      format="%Y%m/%d/", tz="EST"))
comp_collect_analysis$end.date = 
  as.Date(
    as.POSIXct(
      as.character(comp_collect_analysis$end.date), 
      format="%Y%m/%d/", tz="EST"))
...
###### data check
csn_accept_PM_canister = 
  subset(csn_data, csn_data$Analysis == "GC-MS" & csn_data$Collection == "SS.Canister")
csn_accept_PM_MetOne = 
  subset(csn_data, csn_data$Analysis == "Gravemetric" & csn_data$Collection == "MetOne")
...
# start and end date of a method
for (i in 1:nrow(comp_collect_analysis)){
  comp_collect_analysis$start.date[i] = 
    csn_method$Date[min(which(csn_method$comp.collect.analysis == 
                                comp_collect_analysis$comp.collect.analysis[i]))]
  comp_collect_analysis$end.date[i] = 
    csn_method$Date[max(which(csn_method$comp.collect.analysis == 
                                comp_collect_analysis$comp.collect.analysis[i]))]
}
...
comp_collect_analysis.sep = 
  data.frame(str_split_fixed(comp_collect_analysis$comp.collect.analysis, " ", 3))
...
comp_collect_analysis_plot = 
  join(comp_collect_analysis, csn_component_method_use)
comp_collect_analysis_plot$Method = NULL
comp_collect_analysis_plot = 
  comp_collect_analysis_plot[
    with(comp_collect_analysis_plot, 
         order(class, method.No, Serial.No.compo, start.date, Collection, Analysis)), ]
comp_collect_analysis_plot$method.samples = 
  paste(comp_collect_analysis_plot$Collection, 
        comp_collect_analysis_plot$Analysis, 
        comp_collect_analysis_plot$Freq)
comp_collect_analysis_plot$comp.collect.analysis = 
  comp_collect_analysis_plot$Freq = comp_collect_analysis_plot$Collection = 
  comp_collect_analysis_plot$Analysis = comp_collect_analysis_plot$appear.sequence = 
  comp_collect_analysis_plot$ParamCode = comp_collect_analysis_plot$Method = 
  comp_collect_analysis_plot$Unit = comp_collect_analysis_plot$original_list = NULL
...
head(comp_site)
comp_site$comp.collect.analysis = 
  paste(comp_site$CompName, comp_site$Collection, comp_site$Analysis)
...
state_site_method$comp.collect.analysis[
  state_site_method$comp.collect.analysis == 
    "OPC.TOR.88 MetOne IMPROVE_a_TOT "] = 
  "OPC.TOR.88 MetOne IMPROVE_a_TOT"
...
##### temporal method change for components #######
...
combine_4_use = combine_4_use[with(combine_4_use, 
                                   order(class, Serial.No.compo, start.date)), ]
...
collect_analysis_date_a = 
  subset(collect_analysis_date, 
         collect_analysis_date$class == "a")
collect_analysis_date_b = 
  subset(collect_analysis_date, 
         collect_analysis_date$class == "b")
collect_analysis_date_c = 
  subset(collect_analysis_date, 
         collect_analysis_date$class == "c")
collect_analysis_date_a$class = collect_analysis_date_b$class = collect_analysis_date_c$class = NULL
