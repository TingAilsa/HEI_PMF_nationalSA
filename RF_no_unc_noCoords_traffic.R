...
model_input_all_grid = read_fst(paste0(source.test, "_CMAQ_mainlandUS_", cmaq_period, ".fst"))
...
print("Use the sum of NRD and ONR for Traffic")
...
model_input_all_grid$grid_ID[1]; model_input_ini$grid_ID[1]
...
model_input_all_grid_noPMF = 
  subset(model_input_all_grid, is.na(PMF_conc))
print("-----Make suer there is only NAs in model_input_all_grid$PMF_conc-----")
summary(model_input_all_grid_noPMF$PMF_conc)

# Remove column not to use & potential NAs
model_input_use$SiteCode = model_input_all_grid_noPMF$SiteCode = NULL
model_input_use = na.omit(model_input_use)  
model_input_all_grid_noPMF = # still some NAs (1%) in meteorology data
  subset(model_input_all_grid_noPMF, !is.na(th))

# summary(model_input_use); dim(model_input_use)
# summary(model_input_all_grid_noPMF); dim(model_input_all_grid_noPMF)

# Get column order from model_input_use
col_order <- names(model_input_use)

# Reorder model_input_all_grid_noPMF columns
model_input_all_grid_noPMF <- 
  model_input_all_grid_noPMF %>%
  dplyr::select(all_of(col_order))
print("-----If column names are the same in two input datasets-----")
all(names(model_input_use) == names(model_input_all_grid_noPMF))
...
dim(model_input_use); dim(model_input_all_grid_noPMF)

model_input_use$long = model_input_use$lat = 
  model_input_all_grid_noPMF$long = model_input_all_grid_noPMF$lat = NULL
...
model_input_all_commute_NA = 
  dplyr::select(model_input_all_grid_noPMF,  
                Longitude, Latitude, 
...
model_input_all_grid_noPMF_1 = model_input_all_grid_noPMF
# model_input_all_grid_noPMF = model_input_all_grid_noPMF_1
common_cols <- setdiff(names(model_input_commute_mice), c("Longitude", "Latitude"))

model_input_all_grid_noPMF_imputed <- 
  model_input_all_grid_noPMF %>%
...
print("Check if the unmatched commute_time in model_input_all_grid_noPMF is the same as No. of NAs in model_input_all_grid_noPMF_1")
summary(model_input_all_grid_noPMF_1$commute_time == model_input_all_grid_noPMF_imputed$commute_time)
summary(is.na(model_input_all_grid_noPMF_1$commute_time))

# Output the imputed file
write_fst(model_input_all_grid_noPMF_imputed,
...
# Assign the imputed one to model_input_all_grid_noPMF
model_input_all_grid_noPMF = model_input_all_grid_noPMF_imputed

print("-----Last check if there are NAs, or unused columns in model inputs-----")
print("-----model_input_use----")
summary(model_input_use)
print("-----model_input_all_grid_noPMF----")
summary(model_input_all_grid_noPMF)
